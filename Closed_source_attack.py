...
from utils import hash_training_config, setup_wandb, ensure_dir, UniformGridWindowCrop, get_sd_model
...
@hydra.main(version_base=None, config_path="config", config_name="OGJA")
def main(cfg: MainConfig):
    ...
    source_crop = (
        UniformGridWindowCrop(
            out_size=cfg.model.input_res,
            window_ratio=cfg.model.window_ratio,        # 当 area_scale_range 为None时才会采用       
            area_scale_range=cfg.model.area_scale_range,
    ...
    target_crop = (
        UniformGridWindowCrop(
            out_size=cfg.model.input_res,
            window_ratio=cfg.model.window_ratio,
            area_scale_range=cfg.model.area_scale_range,
            stride=cfg.model.stride,
            interval=cfg.model.interval,
        )
...
def attack_imgpair(
    ...
    for path_idx in range(len(path_org)):
        ...
        print(f"Saving image {name} to {folder_to_save}")

        name_lower = name.lower()
        if name_lower.endswith(".jpg") or name_lower.endswith(".jpeg"):
            out_name = os.path.splitext(name)[0] + ".png"
            torchvision.utils.save_image(adv_image[path_idx], os.path.join(folder_to_save, out_name))
        elif name_lower.endswith(".png"):
            torchvision.utils.save_image(adv_image[path_idx], os.path.join(folder_to_save, name))


def log_metrics(pbar, metrics, img_index, cfg, epoch=None):
    """
    ...
    show_keys = ["max_delta", "mean_delta", "local_similarity", "global_similarity"]
    ...
    for k in show_keys:
        if k in metrics:
            kk = alias.get(k, k)
            vv = metrics[k]
            pbar_metrics[kk] = f"{vv:.5f}" if "sim" in k else f"{vv:.3f}"
...
def fgsm_attack(
    ...
    for epoch in pbar:
        ...
        # Log current metrics
        log_metrics(pbar, metrics, img_index, cfg, epoch)
    ...
    log_metrics(pbar, final_metrics, img_index, cfg)
